...
When configured, it also enables viewing MuseScore (MSCZ) files in ExCoda.
...
### Per-File Settings
Settings for GP/GPX, MusicXML, and MSCZ files are stored individually per file, allowing customized configurations:
...
- MuseScore (MSCZ) conversion: Automatically converts MSCZ files to MusicXML format on-the-fly via Registra API for viewing in AlphaTab
