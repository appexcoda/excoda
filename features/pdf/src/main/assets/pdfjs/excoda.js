(function () {
  ...
  function hideToolbarButtons() {
    ...
    style.textContent = `
      ...
      /* Hide add image/stamp button (editor stamp tool) */
      #editorStamp,
      #editorStampButton,
      button[title="Add image"],
      .editorParamsToolbarContainer button[data-l10n-id="editor_stamp"],
      #editorStampParamsToolbar {
        display: none !important;
      }
    `;
    const existing = document.getElementById("excoda-hide-buttons");
    if (existing) {
      existing.remove();
    }
    document.head.appendChild(style);
    console.log("[ExcodaPdf] Toolbar buttons hidden (print, add image)");
  }
