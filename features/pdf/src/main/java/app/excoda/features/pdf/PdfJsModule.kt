...
object PdfJsModule : FileModule {
    ...
    fun initialize(globalSettings: GlobalSettingsRepository) {
    ...
    override fun Content(
        ...
        WithFaceGestures(
            ...
            onGesture = { gestureType ->
                when (gestureType) {
                    app.excoda.core.facegestures.FaceGestureType.MouthStretchRight -> {
                        LxLog.i("PageTurn", "Right mouth stretch → Next page (PDF)")
                        holder.nextPage()
                    }
                    app.excoda.core.facegestures.FaceGestureType.MouthStretchLeft -> {
                        LxLog.i("PageTurn", "Left mouth stretch → Previous page (PDF)")
                        holder.previousPage()
                    }
...
private class PdfJsWebViewHolder(
    ...
    private val assetLoader: WebViewAssetLoader =
        WebViewAssetLoader.Builder()
            ...
            .addPathHandler(
                ...
                if (path != "document.pdf") return@addPathHandler null
                val target = currentUri ?: return@addPathHandler null
                servePdf(target)
            }
            .build()
    ...
    private fun getActualFilename(uri: Uri): String {
        if (uri.toString().isBlank()) {
            LxLog.w("PdfJsDownload", "Cannot get filename for blank URI")
            return "document.pdf"
        }
