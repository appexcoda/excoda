...
class RegistraRepository @Inject constructor(
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                val errorMsg = try {
                    ...
                    LxLog.d("Registra", "Failed to parse error response in search", e)
                    response.message()
                }
            ...
            when {
                ...
                response.code() == 409 -> {
                    ...
                    val conflictResponse = try {
                        ...
                        LxLog.d("Registra", "Failed to parse conflict response in upload", e)
                        null
                    }
                    ...
                    if (conflictResponse?.existingFile != null) {
                        ...
                        val errorMsg = try {
                            ...
                            LxLog.d("Registra", "Failed to parse error response in upload conflict handling", e)
                            "File already exists"
                        }
                        RegistraResult.Error(errorMsg)
                    }
                    ...
                    val errorMsg = try {
                        ...
                        LxLog.d("Registra", "Failed to parse error response in upload", e)
                        response.message()
                    }
                    RegistraResult.Error(errorMsg)
                }
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                val errorMsg = try {
                    ...
                    LxLog.d("Registra", "Failed to parse error response in delete", e)
                    response.message()
                }
                RegistraResult.Error(errorMsg)
            }
    ...
    suspend fun preview(
        ...
        apiKey: String?,
        fileId: String
    ): RegistraResult<ByteArray> = withContext(Dispatchers.IO) {
        try {
            if (host.isNullOrBlank()) {
                return@withContext RegistraResult.Error("Registra host not configured")
            }
            ...
            val api = apiFactory.create(host, apiKey)
            val response = api.preview(fileId)
            ...
            if (!response.isSuccessful) {
                ...
                val errorMsg = try {
                    ...
                    LxLog.d("Registra", "Failed to parse error response in preview", e)
                    response.message()
                }
                return@withContext RegistraResult.Error(errorMsg)
            }

            val body = response.body() ?: return@withContext RegistraResult.Error("Empty response")
            val bytes = body.bytes()
            
            RegistraResult.Success(bytes)
        } catch (e: Exception) {
            LxLog.e("Registra", "Preview failed", e)
            RegistraResult.Error(e.message ?: "Preview failed")
        }
    }
        ...
        try {
            ...
            if (response.isSuccessful && response.body()?.status == "ok") {
                ...
                val errorMsg = try {
                    ...
                    LxLog.d("Registra", "Failed to parse error response in health check", e)
                    response.message()
                }
                RegistraResult.Error(errorMsg)
            }
        } catch (e: java.net.UnknownHostException) {
            LxLog.d("Registra", "Unknown host exception in health check", e)
            RegistraResult.Error("Cannot reach host")
        } catch (e: java.net.ConnectException) {
            LxLog.d("Registra", "Connection exception in health check", e)
            RegistraResult.Error("Connection refused")
        } catch (e: kotlinx.coroutines.TimeoutCancellationException) {
            LxLog.d("Registra", "Timeout exception in health check", e)
