...
class LauncherViewModel @Inject constructor(
    ...
    private fun processUri(uri: Uri) {
        viewModelScope.launch {
            if (uri.toString().isBlank()) {
                LxLog.w("Launcher", "Cannot process empty or blank URI")
                mutableState.update {
                    it.copy(errorMessage = "Invalid file: No URI provided")
                }
                return@launch
            }
            ...
            if (extension == null) {
                ...
                _snackbarMessage.emit("File not found or appears to be missing.")
                return@launch
            }
    ...
    private fun queryDisplayName(uri: Uri): String? {
        if (uri.toString().isBlank()) {
            LxLog.w("Launcher", "Cannot query display name for blank URI")
            return null
        }
        ...
        val cursor: android.database.Cursor? = try {
            ...
            LxLog.i("Launcher", "Permission denial when querying $uri, file is likely missing.")
            null
        } catch (e: Exception) {
            LxLog.w("Launcher", "Exception during contentResolver.query for $uri.", e)
            null
        }

        if (cursor == null) {
            LxLog.w("Launcher", "ContentResolver returned a null cursor for $uri.")
            return null
        }
        ...
        try {
            if (cursor.moveToFirst()) {
                ...
                if (nameIndex != -1) {
                    ...
                    LxLog.w("Launcher", "Display name column not found for $uri.")
                }
            }
        } catch (e: Exception) {
            LxLog.e("Launcher", "Exception while reading from cursor for $uri.", e)
        } finally {
            cursor.close()
        }
        return result
