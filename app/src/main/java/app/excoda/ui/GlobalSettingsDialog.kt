...
import app.excoda.core.settings.QrScannerDialog
import app.excoda.features.registra.CertificateInfo
import app.excoda.features.registra.ConnectionResult
...
fun GlobalSettingsDialog(
    ...
    registraConnectionChecker: RegistraConnectionChecker,
    onDismiss: () -> Unit
) {
    ...
    val showCameraPreview by repository.showCameraPreview.collectAsState(initial = true)
    var localShowCameraPreview by remember(showCameraPreview) { mutableStateOf(showCameraPreview) }
    val autoEnableGestures by repository.autoEnableGestures.collectAsState(initial = false)
    var localAutoEnableGestures by remember(autoEnableGestures) { mutableStateOf(autoEnableGestures) }
    val gestureConsentGiven by repository.gestureConsentGiven.collectAsState(initial = false)
    ...
    val cameraPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (isGranted) {
            scope.launch {
                repository.setAutoEnableGestures(true)
                localAutoEnableGestures = true
            }
        } else {
            scope.launch {
                snackbarHostState.showSnackbar("Camera permission is required for auto-enable gestures")
            }
        }
    }
                ...
                Box(
                    ...
                    when (selectedTab) {
                        SettingsTab.GESTURES -> GesturesTab(
                            ...
                            showCameraPreview = localShowCameraPreview,
                            onShowCameraPreviewChange = {
                                localShowCameraPreview = it
                                scope.launch { repository.setShowCameraPreview(it) }
                            },
                            autoEnableGestures = localAutoEnableGestures,
                            onAutoEnableGesturesChange = { enabled ->
                                if (enabled && !gestureConsentGiven) {
                                    showGestureConsent = true
                                } else if (enabled) {
                                    cameraPermissionLauncher.launch(Manifest.permission.CAMERA)
                                } else {
                                    localAutoEnableGestures = false
                                    scope.launch { repository.setAutoEnableGestures(false) }
                                }
                            }
                        )
                        SettingsTab.REGISTRA -> RegistraTab(
                            ...
                            onCheck = {
                                scope.launch {
                                    isChecking = true
                                    val result = registraConnectionChecker.checkConnection(
                                        localRegistraHost,
                                        localRegistraApiKey,
    ...
    if (showGestureConsent) {
        GestureConsentDialog(
            onAccept = {
                scope.launch {
                    repository.setGestureConsentGiven(true)
...
private fun GesturesTab(
    ...
    onShowCameraPreviewChange: (Boolean) -> Unit,
    autoEnableGestures: Boolean,
    onAutoEnableGesturesChange: (Boolean) -> Unit
) {
    Column(
        ...
        Row(
            ...
            Checkbox(
                ...
                onCheckedChange = onShowCameraPreviewChange
            )
        ...
        Row(
            ...
            Checkbox(
                checked = autoEnableGestures,
                onCheckedChange = onAutoEnableGesturesChange
            )
            Spacer(modifier = Modifier.width(8.dp))
            Text(
                text = "Auto-enable gestures",
                style = MaterialTheme.typography.bodyLarge
            )
        }
