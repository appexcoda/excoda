...
class BlendShapeDetector(
    ...
    private fun detectMouthMovements(scores: Map<String, Float>): Map<FaceGestureType, Float> {
        return buildMap {
            ...
            if (mouthRight > 0.15f) {
                LxLog.d("BlendShapes", "MouthStretchRight: ${String.format(Locale.US, "%.2f", mouthRight)}")
                put(FaceGestureType.MouthStretchRight, mouthRight)
            }

            val mouthLeft = scores["mouthLeft"] ?: 0f
            if (mouthLeft > 0.15f) {
                LxLog.d("BlendShapes", "MouthStretchLeft: ${String.format(Locale.US, "%.2f", mouthLeft)}")
                put(FaceGestureType.MouthStretchLeft, mouthLeft)
            }
        }
    }
